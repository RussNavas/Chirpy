...
const updateRevokeToken = `-- name: UpdateRevokeToken :one
...
func (q *Queries) UpdateRevokeToken(ctx context.Context, token string) (RefreshToken, error) {
	row := q.db.QueryRowContext(ctx, updateRevokeToken, token)
	var i RefreshToken
	err := row.Scan(
		&i.Token,
		&i.CreatedAt,
